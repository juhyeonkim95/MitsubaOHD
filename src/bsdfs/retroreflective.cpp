 ...
 * \end{table}
 */
class SmoothRetroreflective : public BSDF {
public:
    SmoothRetroreflective(const Properties &props) : BSDF(props) {
    ...
    SmoothRetroreflective(Stream *stream, InstanceManager *manager)
            : BSDF(stream, manager) {
    ...
    std::string toString() const {
        ...
        oss << "SmoothRetroreflective[" << endl
            << "  id = \"" << getID() << "\"," << endl
...
*/
class SmoothRetroreflectiveShader : public Shader {
public:
    SmoothRetroreflectiveShader(Renderer *renderer, const Texture *specularReflectance,
...
Shader *SmoothRetroreflective::createShader(Renderer *renderer) const {
    return new SmoothRetroreflectiveShader(renderer,
        m_specularReflectance.get(), m_eta, m_k);
}

MTS_IMPLEMENT_CLASS(SmoothRetroreflectiveShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(SmoothRetroreflective, false, BSDF)
MTS_EXPORT_PLUGIN(SmoothRetroreflective, "Smooth retroreflective");
MTS_NAMESPACE_END
