...
/*!\plugin{RoughRetroreflective}{Rough conductor material}
 * \order{7}
 * \icon{bsdf_RoughRetroreflective}
 * \parameters{
 ...
 *         {bsdf_RoughRetroreflective_copper.jpg}
 *     \rendering{Vertically brushed aluminium (Anisotropic Phong,
 *         $\alpha_u=0.05,\ \alpha_v=0.3$), see
 *         \lstref{RoughRetroreflective-aluminium}}
 *         {bsdf_RoughRetroreflective_anisotropic_aluminium.jpg}
 * }
 *
 ...
 * \vspace{4mm}
 * \begin{xml}[caption={A material definition for brushed aluminium}, label=lst:RoughRetroreflective-aluminium]
 * <bsdf type="RoughRetroreflective">
 ...
 */
class RoughRetroreflective : public BSDF {
public:
    RoughRetroreflective(const Properties &props) : BSDF(props) {
    ...
    RoughRetroreflective(Stream *stream, InstanceManager *manager)
     : BSDF(stream, manager) {
 ...
 */
class RoughRetroreflectiveShader : public Shader {
public:
    RoughRetroreflectiveShader(Renderer *renderer, const Texture *specularReflectance,
...
Shader *RoughRetroreflective::createShader(Renderer *renderer) const {
    return new RoughRetroreflectiveShader(renderer,
        m_specularReflectance.get(), m_alphaU.get(), m_alphaV.get(), m_eta, m_k);
}

MTS_IMPLEMENT_CLASS(RoughRetroreflectiveShader, false, Shader)
MTS_IMPLEMENT_CLASS_S(RoughRetroreflective, false, BSDF)
MTS_EXPORT_PLUGIN(RoughRetroreflective, "Rough conductor BRDF");
MTS_NAMESPACE_END
