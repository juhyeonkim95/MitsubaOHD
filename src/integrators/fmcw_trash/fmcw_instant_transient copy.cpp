 ...
 * }
 */
class MIFMCWInstantTransientPathTracer : public MonteCarloIntegrator, public FMCWInterface {
public:
    MIFMCWInstantTransientPathTracer(const Properties &props)
    ...
    MIFMCWInstantTransientPathTracer(Stream *stream, InstanceManager *manager)
    ...
    std::vector<std::tuple<Spectrum, Float, Float, Float>> Li_helper(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        Vector prev_velocity = Vector(0.0f);
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance) {
                ...
                if(m_use_collimated_light && rRec.depth > 1){
                    ...
                    if(!scene->rayIntersect(ray_temp, its_temp)){
                        ...
                        if (!bsdfVal.isZero() && (!m_strictNormals
                            ...
                            Vector h = normalize(-ray.d + direction);
                            Float new_velocity = 2 * dot(velocity, h) * dot(-ray.d, h);

                            Vector h2 = normalize(-direction-first_dir);
                            Float new_velocity2 = 2 * dot(first_velocity, h2) * dot(-direction, h2);

                            Float em_velocity = path_velocity + new_velocity + new_velocity2;
                    ...
                    if (!bsdfVal.isZero() && (!m_strictNormals
                        ...
                        Float new_velocity = 2 * dot(velocity, -ray.d);
                        Float em_velocity = path_velocity + new_velocity;
                ...
                if (!value.isZero()) {
                    ...
                    if (!bsdfVal.isZero() && (!m_strictNormals
                        ...
                        Vector h = normalize(-ray.d + dRec.d);
                        Float new_velocity = 2 * dot(velocity, h) * dot(-ray.d, h);
            ...
            // transmission?
            if(bRec.sampledType & BSDF::ETransmission){
                Vector h = normalize(prev_ray.d - bRec.eta * ray.d);
                path_velocity += dot(velocity, h) * dot(-prev_ray.d + bRec.eta * ray.d, h);
            } else {
                Vector h = normalize(-prev_ray.d + ray.d);
                path_velocity += 2 * dot(velocity, h) * dot(-prev_ray.d, h);
            }

            // (dot(prev_its.geoFrame.n, -prev_ray.d) + dot(prev_its.geoFrame.n, ray.d)) / dot(prev_its.geoFrame.n, h);
    ...
    std::string toString() const {
        ...
        oss << "MIFMCWInstantTransientPathTracer[" << endl
...
MTS_IMPLEMENT_CLASS_S(MIFMCWInstantTransientPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIFMCWInstantTransientPathTracer, "MI path tracer");
MTS_NAMESPACE_END
