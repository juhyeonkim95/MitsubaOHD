 ...
 * }
 */
class MITransientPathTracer : public MonteCarloIntegrator {
public:
    MITransientPathTracer(const Properties &props)
        : MonteCarloIntegrator(props) {
            m_bin = props.getInteger("bin", 128);
    ...
    MITransientPathTracer(Stream *stream, InstanceManager *manager)
        : MonteCarloIntegrator(stream, manager) { }

    std::tuple<size_t, Float> path_length_to_index(Float path_length) const {
        Float idx = (path_length - m_min_distance) / (m_max_distance - m_min_distance) * m_bin;
    ...
    std::vector<std::tuple<Spectrum, Float, Float>> Li_helper(const RayDifferential &r, RadianceQueryRecord &rRec) const {
        ...
        std::vector<std::tuple<Spectrum, Float, Float>> results;
            ...
            if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
                ...
                if(m_use_collimated_light && rRec.depth > 1){
                    ...
                    if(rRec.depth == 2 || !scene->rayIntersect(ray_temp, its_temp)){
                        ...
                        if (!bsdfVal.isZero() && (!m_strictNormals
                            ...
                            results.push_back({Li, path_pdf, em_path_length});
                ...
                if (!value.isZero()) {
                    ...
                    if (!bsdfVal.isZero() && (!m_strictNormals
                        ...
                        results.push_back({Li, path_pdf * dRec.pdf, em_path_length});
    ...
    std::string toString() const {
        ...
        oss << "MITransientPathTracer[" << endl
    ...
    void renderBlock(const Scene *scene,
        ...
        Float *temp = (Float *) alloca(sizeof(Float) * (SPECTRUM_SAMPLES * (m_bin) + 2));
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                temp[SPECTRUM_SAMPLES * m_bin + 1] = 1.0f;
...
MTS_IMPLEMENT_CLASS_S(MITransientPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MITransientPathTracer, "MI path tracer");
MTS_NAMESPACE_END
