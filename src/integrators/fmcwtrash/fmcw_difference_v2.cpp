 ...
 * }
 */
class MIFMCWDynamicDifferencePathTracer : public MonteCarloIntegrator {
public:
    ...
    std::vector<Spectrum> Li_helper_approx(
        ...
        while(rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            // Accumulate nee Li
            {
                ...
                for(int i=0; i<m_M; i++){
                    ...
                    Float em_path_length = (1-x) * path1.em_path_length + x * path2.em_path_length;
                    Float em_modulation_weight = get_fmcw_weight(time, em_path_length);

                    // (2) interpolate f(x)
                    Spectrum f = (1-x) * f_1 + x * f_2;

                    // (3) accumulate
                    Lis.at(i) += f * em_modulation_weight;
                }
            }
    ...
    void renderBlock(const Scene *scene,
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                std::vector<Spectrum> Lis_approx = Li_helper_approx(r1, r2, rRec);

                std::vector<Spectrum> Lis_gt(m_M);
                ...
                for(size_t i=0; i<m_M; i++){
                    Spectrum result = spec_1 * Lis_approx.at(i);
                    for (int l = 0; l<SPECTRUM_SAMPLES; ++l)
                        temp[offset++] = result[l];
                }
                for(size_t i=0; i<m_M; i++){
                    Spectrum result = Lis_gt.at(i);
                    for (int l = 0; l<SPECTRUM_SAMPLES; ++l)
                        temp[offset++] = result[l];
                }
