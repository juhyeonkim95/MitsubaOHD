...
public:
    ...
    std::pair<Spectrum, bool> Li_helper(
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for(size_t m=0;m<m_M+1; m++){
                    ...
                    auto [spec, valid] = Li_helper(r1, r2, rRec);
                    Lis.at(m) = spec * spec_1;
                    Lis_valid.at(m) = valid;
                    total_valid &= valid;
                }
                sampler->advance();
                // Spectrum diffsum(0.0f);
                // if(Lis_valid.at(m_M))
                // {
                //     Float valid_count = 0.0;
                //     for(size_t m=0;m<m_M+1; m++){
                //         Spectrum Lim = Lis.at(m);
                //         bool valid = Lis_valid.at(m);
                //         if(valid){
                //             Float x = m / (Float) (m_M);
                //             Spectrum Limapprox = (1-x) * Lis.at(0) + x * Lis.at(m_M);
                //             Spectrum diff = (Lim - Limapprox);
                //             diff *= diff;
                //             diffsum += diff;
                //   
                //          valid_count += 1;
                ...
                // diffsum = (Lis.at(0) - Lis.at(m_M));

                int offset = 0;
                for (int l = 0; l<SPECTRUM_SAMPLES; ++l)
                    temp[offset++] = total_valid ? 1.0 : 0.0;
                temp[offset++] = rRec.alpha;
                temp[offset] = Float(Lis_valid.at(0));
                ...
                //         Spectrum Lim = Lis.at(m);
                //         for (int l = 0; l<SPECTRUM_SAMPLES; ++l)
                //             temp[offset++] = 1.0;//Lim[l]; //std::sqrt(diffsum[l], result);
                //     }
                //     temp[offset++] = rRec.alpha;
                //     temp[offset] = 1.0f;
                //     block->put(samplePos, temp);
                ...
                //         Spectrum Lim = Lis.at(m);
                //         for (int l = 0; l<SPECTRUM_SAMPLES; ++l)
                //             temp[offset++] = 0.0; //std::sqrt(diffsum[l], result);
                //     }
                //     temp[offset++] = rRec.alpha;
                //     temp[offset] = 1.0f;
                //     block->put(samplePos, temp);
