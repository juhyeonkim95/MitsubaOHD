...
public:
    ...
    std::tuple<Spectrum, Float, bool> Li_helper(
        ...
        while(rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            // Accumulate nee Li
            if((rRec.depth + 1) == m_target_depth){
                ...
                return {f_2, path2.em_path_length, path2.neeEmitterValue[0] > 0.0f};

                // Float difference_ratio = std::abs(f_2[0] / (f_1[0] + 1e-10) - 1);
                // Float difference_ratio2 = std::abs(path2.em_path_length / path1.em_path_length - 1);
                
                // bool consistency = path1.its.isValid() && path2.its.isValid();
                // consistency &= dot(path1.its.shFrame.n, path2.its.shFrame.n) > 0.8;
                // consistency &= (path1.its.instance == path2.its.instance);
                // consistency &= (path1.neeEmitterValue[0] > 0.0 && path2.neeEmitterValue[0] > 0.0);
                // consistency &= (difference_ratio < 0.1);
                
                // // consistency &= (path1.path_throughput[0] )
                // // bool consistent = difference_ratio < 0.1 && difference_ratio2 < 0.1;

                // if(path2.neeEmitterValue[0] > 0.0 && consistency)
                //     return {f_1, true};
            }
    ...
    void renderBlock(const Scene *scene,
        ...
        Float *temp = (Float *) alloca(sizeof(Float) * (SPECTRUM_SAMPLES + 2));
        std::vector<Spectrum> Lis(m_M+1);
        std::vector<Float> path_lengths(m_M+1);
        std::vector<bool> Lis_valid(m_M+1);
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for(size_t m=0;m<m_M+1; m++){
                    ...
                    auto [spec, path_length, valid] = Li_helper(r1, r2, rRec);

                    Lis.at(m) = spec * spec_1;
                    path_lengths.at(m) = path_length;
                    Lis_valid.at(m) = valid;
                    total_valid &= valid;
                }
                sampler->advance();
                ...
                //l_var = std::sqrt(l_var);
                //L_var = std::sqrt(L_var);

                // for(size_t m=0;m<m_M+1; m++){
                //     Float x = m / (Float) (m_M);
                //     Float Li_approx = (1-x) * L0 + x * L1;
                //     Float path_length_approx = (1-x) * l0 + x * l1;
                //     Float Li = Lis.at(m)[0];
                //     Float path_length = path_lengths.at(m);
                //     Float Li_diff = Li - Li_approx;
                //     Float path_length_diff = path_length - path_length_approx;
                //     l_var += path_length_diff * path_length_diff;
                //     L_var += Li_diff * Li_diff;
                // }
