...
public:
    ...
    void renderBlock(const Scene *scene,
        ...
        Float *temp = (Float *) alloca(sizeof(Float) * (3 + 2));
        ...
        std::vector<Float> Fs_accumulated_gt(m_M+1);
        std::vector<Float> Fs_accumulated_ap(m_M+1);
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for(int depth_idx = 0; depth_idx < m_maxDepth; depth_idx++){
                    ...
                    // (3) error for final signal
                    for(size_t m=0;m<m_M+1; m++){
                        Float x = m / (Float) (m_M);
                        Float t = x * m_T;
                        Float L_ap = L0 * (1-x) + L1 * x;
                        Float l_ap = l0 * (1-x) + l1 * x;
                        Float L_gt = Lis.at(m)[0];
                        Float l_gt = path_lengths.at(m);
                        Float f_ap = L_ap * get_fmcw_weight(t, l_ap);
                        Float f_gt = L_gt * get_fmcw_weight(t, l_gt);

                        Fs_accumulated_gt.at(m) += f_ap;
                        Fs_accumulated_ap.at(m) += f_gt;
                    }
                }
