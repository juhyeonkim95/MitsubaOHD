 ...
 * }
 */
class MIFMCWStaticPathTracer : public MonteCarloIntegrator {
public:
    ...
    std::vector<Spectrum> Li_helper(const RayDifferential &r, RadianceQueryRecord &rRec) const {
    ...
    void renderBlock(const Scene *scene,
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                std::vector<Spectrum> Lis = Li_helper(sensorRay, rRec);

                int array_offset = 0;
                for(size_t i=0; i<m_M; i++){
                    Spectrum result = spec * Lis.at(i);
