 ...
 * }
 */
class MIFMCWDynamicPathTracer : public MonteCarloIntegrator {
public:
    ...
    void renderBlock(const Scene *scene,
        ...
        // temporal float array to store only one sample
        Float *temp = (Float *) alloca(sizeof(Float) * (SPECTRUM_SAMPLES));
        Float *temp2 = (Float *) alloca(sizeof(Float) * 2);
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for(size_t m=0; m < m_M; m++){
                ...
                for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                    ...
                    block->put_single(samplePos, m * SPECTRUM_SAMPLES, SPECTRUM_SAMPLES, temp);

                    temp2[0] = rRec.alpha;
                    temp2[1] = 1.0f;
                    block->put_single(samplePos, m_M * SPECTRUM_SAMPLES, 2, temp2);

                    //Li_m += spec;
                    sampler->advance();
                }
                
                //Li_m /= sampler->getSampleCount();
                //Lis.at(m) = Li_m;
            }
            // block->put_single(samplePos, m, SPECTRUM_SAMPLES, temp);
