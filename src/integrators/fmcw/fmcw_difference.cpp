 ...
 * }
 */
class MIFMCWDynamicDifferencePathTracer : public MonteCarloIntegrator, public FMCWInterface {
public:
    MIFMCWDynamicDifferencePathTracer(const Properties &props)
        ...
        printf("\tspatial_correlation: %s\n", m_spatial_correlation_mode.c_str());
    }

    /// Unserialize from a binary data stream
    MIFMCWDynamicDifferencePathTracer(Stream *stream, InstanceManager *manager)
    ...
    std::vector<std::tuple<Spectrum, Spectrum, Float, Float>> Li_helper_correlate(
        ...
        std::vector<std::tuple<Spectrum, Spectrum, Float, Float>> results;
        ...
        path1.m_use_collimated_light = m_use_collimated_light;

        // path 2
        RayDifferential ray2(r2);
        rRec2.rayIntersect(ray2);
        PathTracePart path2(ray2, rRec2, 1);
        path2.m_use_collimated_light = m_use_collimated_light;
        ...
        while(path1.rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            // Accumulate nee Li
            {
                ...
                results.push_back({f_1, f_2, path1.em_path_length, path2.em_path_length});
            }
        ...
        while (rRec.depth <= m_maxDepth || m_maxDepth < 0) {
            ...
            if (rRec.type & RadianceQueryRecord::EDirectSurfaceRadiance &&
                ...
                if(m_use_collimated_light && rRec.depth > 1){
                    ...
                    if(rRec.depth == 2 || !scene->rayIntersect(ray_temp, its_temp)){
                        Spectrum value(1.0);
    ...
    std::string toString() const {
        ...
        oss << "MIFMCWDynamicDifferencePathTracer[" << endl
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                Spectrum spec_2 = sensor->sampleRayDifferential(
                    r2, samplePos, apertureSample, m_T);
                r2.scaleDifferential(diffScaleFactor);
                r2.setTime(m_T);

                // (1) Linear approx
                std::vector<std::tuple<Spectrum, Spectrum, Float, Float>> results = Li_helper_correlate(r1, r2, rRec);
                for(size_t j=0; j<(2 * m_M * SPECTRUM_SAMPLES + 2); j++){
                    temp[j] = 0.0;
                }
                for (size_t j=1; j<results.size(); j++){
                    auto[L1, L2, d1, d2] = results.at(j);
                    L1 = L1 * spec_1;
                    L2 = L2 * spec_1;
                    ...
                    for(size_t i=0; i<m_M; i++){
                        ...
                        // (1) interpolate path length
                        Float d = (1-x) * d1 + x * d2;
                        Float fmcw_weight = get_fmcw_weight(time, d);

                        // (2) interpolate f(x)
                        Spectrum f = (1-x) * L1 + x * L2;

                        // (3) accumulate
                        for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                            temp[i * SPECTRUM_SAMPLES + l] += fmcw_weight * f[l];
                        }
                    }
                }
                ...
                for(size_t m=0; m < m_M; m++){
                    ...
                    std::vector<std::tuple<Spectrum, Float>> results = Li_helper(sensorRay, rRec);
                    for (size_t j=1; j<results.size(); j++){
                        auto[Li, path_length] = results.at(j);
                        Li = Li * spec;
                        if(m_use_amplitude){
                            for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                                Li[l] = std::sqrt(Li[l]);
                        ...
                        Float fmcw_weight = get_fmcw_weight(time, path_length);
                        for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                            temp[(m + m_M) * SPECTRUM_SAMPLES + l] += fmcw_weight * Li[l];
                        }
                    }
                }
...
MTS_IMPLEMENT_CLASS_S(MIFMCWDynamicDifferencePathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIFMCWDynamicDifferencePathTracer, "MI path tracer");
MTS_NAMESPACE_END
