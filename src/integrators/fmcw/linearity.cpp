 ...
 * }
 */
class MILinearityEvaluator : public MonteCarloIntegrator {
public:
    MILinearityEvaluator(const Properties &props)
        : MonteCarloIntegrator(props) {
        m_T = props.getFloat("T", 7.33);  // micro second (1e-6)
        m_target_depth = props.getInteger("target_depth", 2);  // micro second (1e-6)
        ...
        m_output_mode = props.getString("output_mode", "single");

        printf("Output mode: %s\n", m_output_mode.c_str());
        printf("Spatial correlation mode: %s\n", m_spatial_correlation_mode.c_str());
    }

    /// Unserialize from a binary data stream
    MILinearityEvaluator(Stream *stream, InstanceManager *manager)
    ...
    std::vector<std::tuple<Spectrum, Float, bool>> Li_helper(
            const RayDifferential &r1,
            const RayDifferential &r2, 
            RadianceQueryRecord &rRec
        ) const {

        std::vector<std::tuple<Spectrum, Float, bool>> results;
        ...
        while(rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            consistency &= path1.its.isValid() && path2.its.isValid();
            consistency &= dot(path1.its.shFrame.n, path2.its.shFrame.n) > 0.8;
            consistency &= (path1.its.instance == path2.its.instance);
            ...
            // Accumulate nee Li
            {
                Spectrum f_1 = path1.neeEmitterValue * path1.path_throughput_nee;
                Spectrum f_2 = path2.neeEmitterValue * path2.path_throughput_nee;
                Float p11 = path1.path_pdf_as(path1);
                Float p22 = path2.path_pdf_as(path2);
                if(p11 > 0)
                    f_1 /= p11;
                if(p22 > 0)
                    f_2 /= p22;
                results.push_back({f_2, path2.em_path_length, consistency && path2.neeEmitterValue[0] > 0.0f});
            }
    ...
    void renderBlock(const Scene *scene,
        ...
        // Float *temp = (Float *) alloca(sizeof(Float) * (SPECTRUM_SAMPLES * (m_M + 1) + 2));
        Float *temp;
        if(strcmp(m_output_mode.c_str(), "single") == 0){
            temp = (Float *) alloca(sizeof(Float) * (SPECTRUM_SAMPLES + 2));
        } else if(strcmp(m_output_mode.c_str(), "transient") == 0){
            temp = (Float *) alloca(sizeof(Float) * (SPECTRUM_SAMPLES * (m_M + 1) * 2 + 2));
        }

        std::vector<std::vector<Spectrum>> Lis_total(m_maxDepth, std::vector<Spectrum>(m_M+1));
        std::vector<std::vector<Float>> path_lengths_total(m_maxDepth, std::vector<Float>(m_M+1));
        std::vector<std::vector<bool>> Lis_valid_total(m_maxDepth, std::vector<bool>(m_M+1));
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for(size_t m=0;m<m_M+1; m++){
                    sampler->loadSavedState();
                    Float time = m / (Float) (m_M) * m_T;
                    ...
                    Spectrum spec_1 = sensor->sampleRayDifferential(
                        r1, samplePos, apertureSample, 0.0);
                    r1.scaleDifferential(diffScaleFactor);
                    r1.setTime(0.0);

                    RayDifferential r2;
                    Spectrum spec_2 = sensor->sampleRayDifferential(
                        r2, samplePos, apertureSample, time);
                    r2.scaleDifferential(diffScaleFactor);
                    r2.setTime(time);

                    std::vector<std::tuple<Spectrum, Float, bool>> result = Li_helper(r1, r2, rRec);

                    for(int depth_idx=0; depth_idx<m_maxDepth; depth_idx++){
                        if(depth_idx < result.size()){
                            auto [spec, path_length, valid] = result.at(depth_idx);
                            Lis_total.at(depth_idx).at(m) = spec * spec_1;
                            path_lengths_total.at(depth_idx).at(m) = path_length;
                            Lis_valid_total.at(depth_idx).at(m) = valid;
                        } else {
                            Lis_total.at(depth_idx).at(m) = Spectrum(0.0);
                            path_lengths_total.at(depth_idx).at(m) = 0.0;
                            Lis_valid_total.at(depth_idx).at(m) = false;
                        }
                    }
                }
                sampler->advance();

                // (1) put accumulated variance on L, pathlenght and validity
                if(strcmp(m_output_mode.c_str(), "single") == 0){
                for(int depth_idx = 0; depth_idx < m_maxDepth; depth_idx++){
                    std::vector<Float> path_lengths = path_lengths_total.at(depth_idx);
                    std::vector<Spectrum> Lis = Lis_total.at(depth_idx);
                    std::vector<bool> Lis_valid = Lis_valid_total.at(depth_idx);

                    Float l0 = path_lengths.at(0);
                    Float l1 = path_lengths.at(m_M);
                    Float L0 = Lis.at(0)[0];
                    Float L1 = Lis.at(m_M)[0];
                    Float l_var = 0.0;
                    Float L_var = 0.0;
                    bool valid_path = true;
                    for(size_t m=0;m<m_M+1; m++){
                        valid_path &= Lis_valid.at(m);
                    }
                    // (1) error for path length
                    if(std::abs(l0 - l1) < 1e-8){
                        for(size_t m=0;m<m_M+1; m++){
                            Float path_length_approx = (l0 == 0.0) ? 0 : 0.5;
                            Float path_length = (l0 == 0.0) ? 0 : path_lengths.at(m) / l0 * 0.5;
                            path_length = math::clamp(path_length, 0.0, 1.0);
                            Float path_length_diff = path_length - path_length_approx;
                            l_var += path_length_diff * path_length_diff;
                        }
                    } else {
                        for(size_t m=0;m<m_M+1; m++){
                            Float x = m / (Float) (m_M);
                            Float path_length_approx = x;
                            Float path_length = (path_lengths.at(m) - l0) / (l1 - l0);
                            path_length = math::clamp(path_length, 0.0, 1.0);
                            Float path_length_diff = path_length - path_length_approx;
                            l_var += path_length_diff * path_length_diff;
                        }
                    ...
                    if(std::abs(L0 - L1) < 1e-8){
                        for(size_t m=0;m<m_M+1; m++){
                            Float L_approx = (L0 == 0.0) ? 0.0 : 0.5;
                            Float L = (L0 == 0.0) ? 0.0 : Lis.at(m)[0] / L0 * 0.5;
                            L = math::clamp(L, 0.0, 1.0);
                            Float L_diff = L - L_approx;
                            L_var += L_diff * L_diff;
                        }
                    } else {
                        for(size_t m=0;m<m_M+1; m++){
                    ...
                    if(!Lis_valid.at(0)){
                        ...
                        temp[offset++] = L0 * L_var;
                        temp[offset++] = L0 * l_var;
                        temp[offset++] = L0 * Float(valid_path);
                ...
                // (2) put signal L, path length over time
                else if(strcmp(m_output_mode.c_str(), "transient") == 0){
                for(int depth_idx = 0; depth_idx < m_maxDepth; depth_idx++){
                    std::vector<Float> path_lengths = path_lengths_total.at(depth_idx);
                    std::vector<Spectrum> Lis = Lis_total.at(depth_idx);
                    ...
                    // for(size_t m=0;m<m_M+1; m++){
                    //     Float L_cropped = (L0 == 0.0) ? 0.0 : Lis.at(m)[0];
                    //     temp[offset++] = math::clamp(L_cropped, 0.0, 1.0);
                    // }
                    // // put path length over time
                    // for(size_t m=0;m<m_M+1; m++){
                    //     temp[offset++] = (L0 == 0.0) ? 0.0 : L0 * path_lengths.at(m) / l0;
                    //     // temp[offset++] = L0 * path_lengths.at(m);
                    // }
...
MTS_IMPLEMENT_CLASS_S(MILinearityEvaluator, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MILinearityEvaluator, "MI path tracer");
MTS_NAMESPACE_END
