...
public:
    ...
    std::vector<std::tuple<Spectrum, Spectrum, Float, Float, Float, Float>> Li_helper(
        ...
        std::vector<std::tuple<Spectrum, Spectrum, Float, Float, Float, Float>> results;
        ...
        while(path1.rRec.depth <= m_maxDepth || m_maxDepth < 0){
            ...
            // Accumulate nee Li
            {
                ...
                results.push_back({f_1, f_2, path1.em_path_length, path2.em_path_length, p11, p22});
            }
    ...
    void renderBlock(const Scene *scene,
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                std::vector<std::tuple<Spectrum, Spectrum, Float, Float, Float, Float>> results = Li_helper(r1, r2, rRec);
                ...
                for (size_t j=0; j<results.size(); j++){
                    auto[L1, L2, d1, d2, p1, p2] = results.at(j);
                    L1 = L1 * spec_1;
                    L2 = L2 * spec_1;
                    ...
                    if(m_use_amplitude){
                        if(m_sqrt_pdf){
                            for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                                if(p1>0) L1[l] = std::sqrt(L1[l] * p1) / p1;
                                if(p2>0) L2[l] = std::sqrt(L2[l] * p2) / p2;
                            }
                        } else {
                            for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                                L2[l] = std::sqrt(L2[l]);
                    ...
                    for(size_t i=0; i<m_M; i++){
                        ...
                        Float fmcw_weight_inverted = get_fmcw_weight_invert(time, d);
                        ...
                        for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                            temp[(m_M + i) * SPECTRUM_SAMPLES + l] += fmcw_weight_inverted * f[l];
                        }
                    }
                }
