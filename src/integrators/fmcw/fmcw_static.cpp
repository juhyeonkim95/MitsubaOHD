 ...
 * }
 */
class MIFMCWStaticPathTracer : public MonteCarloIntegrator, public FMCWInterface {
public:
    MIFMCWStaticPathTracer(const Properties &props)
        : MonteCarloIntegrator(props), FMCWInterface(props) {
    }

    /// Unserialize from a binary data stream
    MIFMCWStaticPathTracer(Stream *stream, InstanceManager *manager)
    ...
    std::string toString() const {
        ...
        oss << "MIFMCWStaticPathTracer[" << endl
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for (size_t j=0; j<results.size(); j++){
                    ...
                    for(size_t i=0; i<m_M; i++){
                        ...
                        Float fmcw_weight = get_fmcw_weight(t, path_length);
                        for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
...
MTS_IMPLEMENT_CLASS_S(MIFMCWStaticPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIFMCWStaticPathTracer, "MI path tracer");
MTS_NAMESPACE_END
