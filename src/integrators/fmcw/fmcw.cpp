...
#define PATH_CORRELATION_NONE 0 
#define PATH_CORRELATION_PIXEL 1
#define PATH_CORRELATION_FULL 2
...
public:
    MIFMCWDynamicPathTracer(const Properties &props)
        : MonteCarloIntegrator(props), FMCWInterface(props) {
            std::string path_correlation = props.getString("path_correlation", "full");
            if (strcmp(path_correlation.c_str(), "full") == 0){
                m_path_correlation_strategy = PATH_CORRELATION_FULL;
            } else if (strcmp(path_correlation.c_str(), "none") == 0){
                m_path_correlation_strategy = PATH_CORRELATION_NONE;
            } else if (strcmp(path_correlation.c_str(), "pixel") == 0){
                m_path_correlation_strategy = PATH_CORRELATION_PIXEL;
            }
            printf("\tpath correlation : %s\n", path_correlation.c_str());
        }
    ...
    void renderBlock(const Scene *scene,
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                if(m_path_correlation_strategy == PATH_CORRELATION_FULL)
                    sampler->saveState();

                for(size_t m=0; m < m_M; m++){
                    if(m_path_correlation_strategy == PATH_CORRELATION_FULL)
                        sampler->loadSavedState();
                    else if (m_path_correlation_strategy == PATH_CORRELATION_NONE){
                        Vector2 jitter = Vector2(rRec.nextSample2D()) * 2.0 - Vector2(1.0);
                        jitter = jitter * m_fov_error + Vector2(0.5);
                        samplePos = Point2(offset) + jitter;
                    }
                    ...
                    for (size_t j=0; j<results.size(); j++){
                        ...
                        for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                            temp[m * SPECTRUM_SAMPLES + l] += fmcw_weight * Li[l];
                        }
                    }
                    if(m_path_correlation_strategy != PATH_CORRELATION_FULL){
                        sampler->advance();
                ...
                if(m_path_correlation_strategy != PATH_CORRELATION_NONE){
                    block->put(samplePos, temp);
                } else {
                    Point2 midPos(Point2(offset) + Vector2(0.5, 0.5));
                    block->put(midPos, temp);
                }
            }
        }
    }
    MTS_DECLARE_CLASS()
private:
    size_t m_path_correlation_strategy;
};
