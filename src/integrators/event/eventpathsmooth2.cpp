 ...
 * }
 */
class MIEventCorrelatedSmoothPathTracer : public MonteCarloIntegrator {
public:
    MIEventCorrelatedSmoothPathTracer(const Properties &props)
    ...
    MIEventCorrelatedSmoothPathTracer(Stream *stream, InstanceManager *manager)
    ...
    Spectrum accumulate_nee_Li(PathTracePart &path1, PathTracePart &path2) const{
        if(path1.path_pdf_nee_as_nee > 0){
            ...
            Float mis_primal_antithetic = 1.0;//miWeight(p11, p12, 1.0);
            ...
            return path1.path_throughput_nee * path1.neeEmitterValue / path_pdf * mis_primal_antithetic * mis_nee_next_ray;
        }
        return Spectrum(0.0);
    }
    ...
    Spectrum accumulate_next_ray_Li(PathTracePart &path1, PathTracePart &path2) const{
        if(path1.hitEmitter){
            ...
            Float path_pdf = p11;
            return path1.path_throughput * path1.hitEmitterValue / path_pdf * mis_primal_antithetic * mis_nee_next_ray;
        }
        return Spectrum(0.0);
    }

    // Trace ray with paired sample (total 2)
    std::vector<std::tuple<Spectrum, Boolean>> Li_helper(
        const RayDifferential &r1,
        const RayDifferential &r2, 
        RadianceQueryRecord &rRec,
        Sampler* perturb_sampler,
        float perturb_amount=0.0,
        bool target_first=false
    ) const {
        std::vector<std::tuple<Spectrum, Boolean>> results(2 * maxDepth);
        
        Spectrum Li(0.0f);
        for(int i=0; i<2 * maxDepth){
            results.at(i) = {Li, false};
        }
        ...
        while(path1.rRec.depth <= m_maxDepth || m_maxDepth < 0){
            //Point2 bsdf_sample = path1.rRec.nextSample2D();
            //path1_next = path1;
            ...
            if (!path1.path_terminated && ((path1.rRec.depth >= m_maxDepth && m_maxDepth > 0)
                || (m_strictNormals && dot(path1.ray.d, path1.its.geoFrame.n)
                    * Frame::cosTheta(path1.its.wi) >= 0))) {
                path1.path_terminated = true;
            }

            if (!path2.path_terminated && ((path2.rRec.depth >= m_maxDepth && m_maxDepth > 0)
            ...
            /* ==================================================================== */
            Point2 nee_sample = path1.rRec.nextSample2D();
            ...
            results.at(2 * path1.rRec.depth + 0) = {Li, !Li.isZero() && consistency};
            ...
            // path1 = path1_next;
            Point2 bsdf_sample = path1.rRec.nextSample2D();
            Point2 bsdf_sample_perturb = perturb_sampler->next2D();
            bsdf_sample.x += perturb_amount * bsdf_sample_perturb.x;
            bsdf_sample.y += perturb_amount * bsdf_sample_perturb.y;
            bsdf_sample.x = fmod(bsdf_sample.x, 1.0);
            bsdf_sample.y = fmod(bsdf_sample.y, 1.0);
            ...
            results.at(2 * path1.rRec.depth + 1) = {Li, !Li.isZero() && consistency};


            path1.rRec.type = RadianceQueryRecord::ERadianceNoEmission;
            path2.rRec.type = RadianceQueryRecord::ERadianceNoEmission;
    ...
    std::string toString() const {
        ...
        oss << "MIEventCorrelatedSmoothPathTracer[" << endl
    ...
    void renderBlock(const Scene *scene,
        ...
        ref<Sampler> perturb_sampler = sampler->clone();
        ...
        uint32_t N_depth = (2 * m_maxDepth);

        Float *temp_per_depth = (Float *) alloca(sizeof(Float) * (m_M * SPECTRUM_SAMPLES * N_depth + 2));
        Float *temp = (Float *) alloca(sizeof(Float) * (m_M * SPECTRUM_SAMPLES  + 2));
        bool *valid = (bool *) alloca(sizeof(bool) * N_depth);
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for(size_t m=0; m < m_M; m++){
                    for(int k=0; k<1; k++){
                        ...
                        Float time = (m + 0.5f) / m_M * (m_end_time - m_start_time) + m_start_time;

                        rRec.newQuery(queryType, sensor->getMedium());

                        RayDifferential r1;
                        Float t1 = (k == 0)? 0.0 : time;
                        Spectrum spec_1 = sensor->sampleRayDifferential(
                            r1, samplePos, apertureSample, t1);
                        r1.scaleDifferential(diffScaleFactor);
                        r1.setTime(t1);

                        RayDifferential r2;
                        Float t2 = (k == 1)? 0.0 : time;
                        Spectrum spec_2 = sensor->sampleRayDifferential(
                            r2, samplePos, apertureSample, t2);
                        r2.scaleDifferential(diffScaleFactor);
                        r2.setTime(t2);

                        std::vector<std::tuple<Spectrum, Boolean>> results = 
                            Li_helper(r1, r2, rRec, perturb_sampler, 0.0, (k == 1));
                        
                        for(int i=0; i<2*maxDepth; i++){
                            valid[m * N_depth + i] = results.at(i).second;
                        }
                        for(int i=0; i<2*maxDepth; i++){
                            for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                                temp_per_depth[(m * N_depth + i) * SPECTRUM_SAMPLES + l] = result[l];
                            }
                        }
                        // for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                        //     temp[m * SPECTRUM_SAMPLES + l] += result[l];
                        // }
                        // result = result * spec_1;
                        // for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                        //     temp[m * SPECTRUM_SAMPLES + l] += result[l];
                        // }
                    }
...
MTS_IMPLEMENT_CLASS_S(MIEventCorrelatedSmoothPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIEventCorrelatedSmoothPathTracer, "MI path tracer");
MTS_NAMESPACE_END
