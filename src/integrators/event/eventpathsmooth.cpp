    ...
    void renderBlock(const Scene *scene,
        ...
        for (size_t i = 0; i<points.size(); ++i) {
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                PathInfo path_prev;
                size_t consecutive_path_count = 0;
                std::vector<size_t> consecutive_path_counts;
                for(size_t m=0; m < m_M; m++){
                    ...
                    std::pair<Spectrum, PathInfo> result = Li_helper(sensorRay, rRec);
                    PathInfo path_curr = result.second;
                    if(m == 0){
                        consecutive_path_count =1;
                    } else {
                        bool consistency = check_path_consistency(path_prev, path_curr);
                        if(consistency){
                            consecutive_path_count +=1;
                        } else {
                            consecutive_path_counts.push_back(consecutive_path_count);
                            consecutive_path_count = 1;
                        }
                    }
                    path_prev = path_curr;

                    Spectrum Li = result.first;
                    Li *= spec;

                    for(size_t l=0; l<SPECTRUM_SAMPLES; l++){
                        temp[m * SPECTRUM_SAMPLES + l] = Li[l];
                    }
                }
                if(consecutive_path_count > 0){
                    consecutive_path_counts.push_back(consecutive_path_count);
                }
