 ...
 * }
 */
class MIEventCorrelatedSinglePathTracer : public MonteCarloIntegrator {
public:
    MIEventCorrelatedSinglePathTracer(const Properties &props)
    ...
    MIEventCorrelatedSinglePathTracer(Stream *stream, InstanceManager *manager)
    ...
    std::string toString() const {
        ...
        oss << "MIEventCorrelatedSinglePathTracer[" << endl
            ...
            for (size_t j = 0; j<sampler->getSampleCount(); j++) {
                ...
                for(size_t m=0; m < m_M; m++){
                    for(int k=0; k<1; k++){
                        ...
                        r2.setTime(t2);
                        float perturb_amount = (k==0)? 0.0 : 0.01;
                        Spectrum result = Li_helper(r1, r2, rRec, perturb_sampler, perturb_amount, (k == 1));
...
MTS_IMPLEMENT_CLASS_S(MIEventCorrelatedSinglePathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIEventCorrelatedSinglePathTracer, "MI path tracer");
MTS_NAMESPACE_END
