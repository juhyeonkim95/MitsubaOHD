 ...
 * }
 */
class MIEventESIMPathTracer : public MonteCarloIntegrator {
public:
    MIEventESIMPathTracer(const Properties &props)
        : MonteCarloIntegrator(props) { 
            ...
            m_event_threshold = props.getFloat("event_threshold", 0.2f);
        }

    /// Unserialize from a binary data stream
    MIEventESIMPathTracer(Stream *stream, InstanceManager *manager)
    ...
    std::string toString() const {
        ...
        oss << "MIEventESIMPathTracer[" << endl
    ...
    std::pair<Float, Float> find_next_event(size_t N, float start_time, 
        ...
        Spectrum L1 = single_time(N, start_time + dt, rRec, sensor, sampler, offset);
...
MTS_IMPLEMENT_CLASS_S(MIEventESIMPathTracer, false, MonteCarloIntegrator)
MTS_EXPORT_PLUGIN(MIEventESIMPathTracer, "MI path tracer");
MTS_NAMESPACE_END
